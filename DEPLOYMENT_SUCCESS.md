...
- **Еженедельно в 02:00** (воскресенье) - полный бэкап с очисткой логов  
- **Ежемесячно в 01:00** (1-го числа) - очистка старых логов

```bash
# Установка автоматических бэкапов (выполнить один раз)
cd /home/ubuntu/carebot
...
# Или через PowerShell с Windows машины
.\scripts\backup-database.ps1

# Или через deploy скрипт
.\scripts\deploy-proven.ps1 backup
```
...
# Восстановление без подтверждения
./scripts/restore-database.sh --force

# Или через deploy скрипт
.\scripts\deploy-proven.ps1 restore
```

#### Расположение бэкапов

- **Директория:** `/home/ubuntu/carebot/backups/`
- **SQL дампы:** `carebot_backup_YYYYMMDD_HHMMSS.sql`
- **Бинарные копии:** `carebot_binary_YYYYMMDD_HHMMSS.db`
- **Последний бэкап:** `latest_backup.sql` и `latest_backup.db`
- **Логи:** `/tmp/carebot_backup.log` и `/tmp/carebot_backup_weekly.log`

#### Политика хранения

- **SQL дампы:** 30 дней (ежедневные)
- **Бинарные копии:** 7 дней (еженедельные)
- **Логи бэкапов:** 30 дней

### Обновленный deploy скрипт

Скрипт `deploy-proven.ps1` теперь автоматически:
1. **Создает бэкап** перед развертыванием
2. Поддерживает команды `backup` и `restore`
3. Предотвращает потерю данных при деплое

```powershell
# Полное развертывание с автоматическим бэкапом
.\scripts\deploy-proven.ps1 deploy

# Только создание бэкапа
.\scripts\deploy-proven.ps1 backup
...
.\scripts\deploy-proven.ps1 logs
.\scripts\deploy-proven.ps1 restart
.\scripts\deploy-proven.ps1 stop
```
...
6. **🛡️ НЕ СОЗДАВАЙТЕ НОВЫЕ СИСТЕМЫ БЭКАПОВ** - полная система уже реализована!

### 🚨 ДЛЯ AI АГЕНТОВ: СИСТЕМА БЭКАПОВ ГОТОВА!

**ВНИМАНИЕ:** Автоматическая система резервного копирования полностью реализована и работает!

- ✅ Автоматические ежедневные бэкапы в 03:30
- ✅ Еженедельные полные бэкапы по воскресеньям  
- ✅ Автоматический бэкап перед каждым развертыванием
- ✅ Скрипты восстановления с проверкой целостности
- ✅ Политика хранения 30 дней SQL + 7 дней бинарных
- ✅ PowerShell управление с Windows машины
- ✅ Полная документация в `BACKUP_SYSTEM_COMPLETE.md`

**НЕ СОЗДАВАЙТЕ ДОПОЛНИТЕЛЬНЫЕ СИСТЕМЫ БЭКАПОВ!**
